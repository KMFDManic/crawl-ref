...
int RetroWrapper::init(coord_def *m_windowsz, int *densityNum, int *densityDen)
{
   static int _densityNum = SS_WIDTH;
...
unsigned int RetroWrapper::set_timer(unsigned int interval, wm_timer_callback callback)
{
    // (EMPTY) TODO: Needed for tooltips
    return 0;
}

void RetroWrapper::show_keyboard()
{
   /* TODO - should we implement this for devices not supporting a keyboard? */
}

void RetroWrapper::remove_timer(unsigned int timer_id)
{
}
...
struct ImageBuf
{
        ...
        rows(new unsigned int*[height])
    {
        ...
        memset(pixels, 0, sizeof(unsigned int)*width*height);
    }
